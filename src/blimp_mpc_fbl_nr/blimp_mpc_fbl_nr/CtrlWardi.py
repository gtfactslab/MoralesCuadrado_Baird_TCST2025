...
class CtrlWardi(BlimpController):
    ...
    def get_jax_pred(self, sim):
        ...
        predictor_type = 'rk4' # 'euler / 'rk4' / 'rk3' / 'rk2'
        t0 = time.time()
        ...
        print(f"output time: {time.time()-t0 = }")
        print(f"{outputs = }")
        print(f"{invjacU = }")
        print(f"cond_number: \n{cond_number}")
    ...
    def get_tracking_error(self, n, pred):
        n_plus_lookahead = n + int(self.T_lookahead / self.dT)
        traj_x, traj_y, traj_z, traj_psi = self.traj_x[n_plus_lookahead], self.traj_y[n_plus_lookahead], self.traj_z[n_plus_lookahead], self.traj_psi[n_plus_lookahead]
        if not self.jax_err:
            print("basic error")
            err_x = traj_x - pred[0][0]
            err_y = traj_y - pred[1][0]
            err_z = traj_z - pred[2][0]
            err_psi = self.shortest_path_yaw(traj_psi, pred[3][0])
            base_error = np.array([[err_x, err_y, err_z, err_psi]]).T
        else:
            print("basic jax error")
            base_error = np.array(jax_get_tracking_error(pred, traj_x, traj_y, traj_z, traj_psi))
        return base_error
    
    def get_enhanced_tracking_error(self, n, pred, enhance_term):
        n_plus_lookahead = n + int(self.T_lookahead / self.dT)
        traj_x, traj_y, traj_z, traj_psi = self.traj_x[n_plus_lookahead], self.traj_y[n_plus_lookahead], self.traj_z[n_plus_lookahead], self.traj_psi[n_plus_lookahead]
        traj_xdot, traj_ydot, traj_zdot, traj_psidot = self.traj_x_dot[n_plus_lookahead], self.traj_y_dot[n_plus_lookahead], self.traj_z_dot[n_plus_lookahead], self.traj_psi_dot[n_plus_lookahead]
        if not self.jax_err:
            ...
            err_z = traj_z - pred[2][0]
            err_psi = self.shortest_path_yaw(traj_psi, pred[3][0])
            enhanced_err_x = traj_xdot - enhance_term[0][0]
            enhanced_err_y = traj_ydot - enhance_term[1][0]
            enhanced_err_z = traj_zdot - enhance_term[2][0]
            enhanced_err_psi = traj_psidot - enhance_term[3][0]
            base_error = np.array([[err_x, err_y, err_z, err_psi]]).T
            enhanced_error = np.array([[enhanced_err_x, enhanced_err_y, enhanced_err_z, enhanced_err_psi]]).T
        else:
            print("enhanced jax error")
            base_error, enhanced_error = jax_get_enhanced_tracking_error(pred, enhance_term,
                                                                         traj_x, traj_y, traj_z, traj_psi,
                                                                         traj_xdot, traj_ydot, traj_zdot, traj_psidot)
        return base_error, enhanced_error

    def get_old_error(self, n, pred):
        n_plus_lookahead = n + int(self.T_lookahead / self.dT)
        traj_x, traj_y, traj_z, traj_psi = self.traj_x[n_plus_lookahead], self.traj_y[n_plus_lookahead], self.traj_z[n_plus_lookahead], self.traj_psi[n_plus_lookahead]
        if not self.jax_err:
            err_x = traj_x - pred[0][0]
    ...
    def get_ctrl_action(self, sim):        
        ...
        if self.enhanced_predictor:
            base_error, enhanced_error = self.get_enhanced_tracking_error(n, pred, enhance_term)
        elif not self.enhanced_predictor:
            base_error = self.get_tracking_error(n, pred)
        else:
            raise ValueError("Invalid enhanced predictor value")
        total_err = self.alpha * base_error + enhanced_error if self.enhanced_predictor else self.alpha * base_error
